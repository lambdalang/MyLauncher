...
package com.semicircularmenu.libary;
...
public class SemiCircularRadialMenu extends View {
	...
	public  int SemiCircularRadialMenuIsShow=0;
	...
	public  boolean isMenuTogglePressed = false;
	...
	private int centerRadialColor = Color.YELLOW;
	private int mShadowColor = Color.GRAY;
	private String openMenuText = "Open";
	private String closeMenuText = "Close";
	private String centerMenuText = openMenuText; // Not to be set using setter
													// method
...
//		mBackGroundImage = new ImageView();
	}
	
	@Override
	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		mRadialMenuPaint.setAntiAlias(true); // 璁剧疆鐢荤瑪涓烘姉閿娇
		...
		if (isMenuVisible) {
			...
			if (mMenuItems.size() > 0) {
				...
				float mSweep = 180 / mMenuItems.size();// 濞ｅ浂鍠楅弫锟� 闁革箑妫涘▓鎴﹀箑鐠囨彃袣閹艰揪鎷�
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (isMenuTogglePressed) {
				centerRadialColor = Color.YELLOW;
				if (isMenuVisible) {
					isMenuVisible = false;
		...
		//	SemiCircularRadialMenuIsShow=(SemiCircularRadialMenuIsShow == 0)?1:0;
 ...
 * 自己添加的接口
 * */
private OnMenuVisibleChangeListener visibleListener = null;
	
	public void setOnMenuVisibleChangeListener(OnMenuVisibleChangeListener l)
	{
	    visibleListener = l;
	}
	
	public static interface OnMenuVisibleChangeListener
	{
